...
### Гарантийное обеспечение
  ...
  ГО для открытых позиций вычисляется с учетом направления сделки (покупка/продажа).  
  Отображается в колонке 'actualDepo'.  
...
### Точность вычислений
  ...
  Реализована настройка точности расчета PnL.  
  Точность определения цен открытия и закрытия определяется в модуле Settings, метод  
  function Settings:precision_table()  
  В алгоритмах используется точность из данной таблицы. Если точность не указана, применяется значение по-умолчанию "2".  
  
  На текущий момент есть особенность работы. Для инструментов FORTS пока придется указывать полный код вместе с номером месяца и года, например EDH9.  Далее планируется доработать алгоритм, чтобы указывать только общие символы инструмента, в данном случае ED.  
  Драйвер SQLITE странно отдает данные о цене открытия по некоторым инструментам. Чтобы получить точность 5 пришлось умножить цену на 100 000 в запросе, который получает остатки из ФИФО.  
    CAST(coalesce(q1.price, 0) AS NUMERIC)*100000 AS price  
  
  Реализована аналогичная работа с точностью вычислений для таблицы детальных записей.
  
### Сервисные возможности

  Ручное добавление сделки в ФИФО вынесено в класс Service.  
